...
## 3.7 Компьютерные сети - лекция 2 - Вячеслав Медведев

1. Список сетевых интерфейсов можно под windows посмотреть командой ipconfig<br>
под linux: ip, например, ip link<br>

2. Протокол для распознавания соседей - LLDP<br>
Поставить софт для работы с ним можно командой (Ubuntu): sudo apt install lldpd<br>
Дальше для просмотра соседей используем команду lldpctl, для интерактивной работы с lldpd можно использовать lldpcli<br>

3. Для разделения L2 сети на виртуальные сети используется технология VLAN<br>
Для настройки VLAN в Ubuntu нужен пакет vlan (apt install vlan) <br>

Пример добавления сабинтерфейса с VID 2345 в /etc/network/interfaces<br>
...
4. В Linux для объединения нескольких физических сетевых интерфейсов в один используется bonding<br>

Пример /etc/network/interfaces (два интерфейса eth0, eth1 объединяются в bond0; eth0 - основной, eth1 резервный):<br>
...
Тип бондинга задается ключевым словом bond-mode:

- active-backup (как в примере) - только один из физических портов в бонде активен
- balance-rr - round-robin балансировка интерфейсов
- balance-xor - агрегация на основе операции XOR над MAС-адресами, обеспечивает привязку соединений каждого MAC-адреса назначания к конкретному интерфейсу
- broadcast - рассылает все пакеты по всем интерфейсам бонда
- 802.3ad - агрегация по протоколу LACP
- balance-tlb - агрегация, обеспечивающая балансировку исходящего трафика по всем физическим интерфейсам бонда, а приём только по одному активному
- balance-alb - balance-tlb + балансировка входящего трафика с помощью ARP
<br>
5. /29 маска - 6 адресов (+ адрес сети, + броадкаст-адрес)<br>
В сети /24 можно выделить 32 сети /29<br>

Примеры подсетей /29 внутри подсети 10.10.10.0/24
...
6. 100.64.0.0/26 (62 адреса, не считая служебных)
<br>
7. Ubuntu<br>
arp -e - посмотреть таблицу<br>
arp -d <address> - удалить запись из таблицы<br>
ip -s -s neigh flush all - очистка всей arp-таблицы<br>
 
Windows:<br>
arp -a - посмотреть таблицу<br>
arp -d <address> - удалить запись из таблицы<br>
arp -d * - очистка всей arp-таблицы <br>
