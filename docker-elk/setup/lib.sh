...
function wait_for_elasticsearch {
	local elasticsearch_host="${ELASTICSEARCH_HOST:-es-hot}"

	local -a args=( '-s' '-k' '-D-' '-m15' '-w' '%{http_code}' "https://${elasticsearch_host}:9200/" )
...
function wait_for_builtin_users {
	...
	local -a args=( '-s' '-k' '-D-' '-m15' "https://${elasticsearch_host}:9200/_security/user?pretty" )
